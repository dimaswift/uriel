    ...
    {  
        ...
        {  
            ...
            {  
                ...
                const float3 finalColor = hsv2rgb(sampleField(pos * _GradientThreshold, _GeneCount, _GeneBuffer), 1.0, 1.0);  
                o.worldPos = pos;
                o.color = finalColor;
                return o;  
            }  
            ...
            fixed4 frag(v2f i) : SV_Target  
            {
                ...
                // const float3 normalDir = normalize(i.worldNormal);
                ...
                // const float3 diffuseLighting = ndot_l  * float3(1,1,1);
                // const float ndot_h = saturate(dot(normalDir, halfwayDir));  
                // const float specularValue = saturate(i.value * _SpecularThreshold) * _SpecularMultiplier;  
                // const float3 specularLighting = pow(ndot_h, _Shininess) * specularValue;
                // const float4 diffuseColor = tex2D(_GradientLUT, float2(i.value * _GradientThreshold, 0)) * _GradientMultiplier;
