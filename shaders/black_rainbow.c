...
#define GRID_SIZE 32             // number of cells per row/column
#define FIELD_SIZE 32.0          // virtual resolution
#define NUM_SOURCES 5      // number of oscillators
#define SOURCE_SPEED 100.5         // time influence
#define FREQ_MULT 10.0           // distance scaling
#define HUE_RANGE 5.0         // rainbow cutoff
#define CELL_BORDER 0.00         // 0.0 = full square, 0.5 = no square
#define DENSITY 10.00
// ========== HSV TO RGB ==========
...
float interference(vec2 uv, float t) {
    ...
    for (int i = 0; i < NUM_SOURCES; i++) {
        ...
        vec2 source = iMouse.xy / iResolution.xy + v * DENSITY * vec2(cos(a), sin(a));
        float d = distance(uv, source);
        sum += sin(d * FREQ_MULT - t * SOURCE_SPEED);
    }
    return sum;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 fC = color * mask;
    fragColor = vec4(fC.x, fC.x, fC.x, 1.0);
}
